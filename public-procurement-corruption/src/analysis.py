import numpy as np
import pandas as pd
import matplotlib.pyplot as plt


# ============================================================
# KAIROS CORRUPTION LAB
# EXPERIMENT 6 — INSTITUTIONAL RESPONSE TO DETECTION
#
# Objective:
# Hold the procurement market and all parameters constant,
# vary only the probability of detection p,
# and observe how strategic behavior and Public Value respond.
#
# IMPORTANT:
# This is NOT Monte Carlo.
# We use the exact same 100 tenders and 500 bids generated
# in Experiment 4.
# ============================================================


# ------------------------------------------------------------
# 1. FIXED PARAMETERS
# ------------------------------------------------------------

BRIBE = 20

FIRM_PENALTY = 100
OFFICIAL_PENALTY = 100

PRIVATE_PROFIT_MARGIN = 0.20

HONEST_PUBLIC_VALUE_EXPECTED = 8197.05


# ------------------------------------------------------------
# 2. DETECTION GRID
# ------------------------------------------------------------
#
# p = 0%, 5%, 10%, ..., 100%
# ------------------------------------------------------------

DETECTION_VALUES = np.arange(
    0,
    1.0001,
    0.05
)


# ------------------------------------------------------------
# 3. LOAD THE SAME HONEST MARKET
# ------------------------------------------------------------

market = pd.read_csv(
    "results/honest_market_all_bids.csv"
)


# ------------------------------------------------------------
# 4. VALIDATE MARKET
# ------------------------------------------------------------

assert len(market) == 500

assert (
    market["tender_id"]
    .nunique()
    == 100
)

assert (
    market
    .groupby("tender_id")
    .size()
    .eq(5)
    .all()
)


# ------------------------------------------------------------
# 5. PRIVATE PROFIT
# ------------------------------------------------------------
#
# Same assumption as Experiment 5:
#
# PrivateProfit_i = margin * Price_i
# ------------------------------------------------------------

market["private_profit"] = (
    PRIVATE_PROFIT_MARGIN
    * market["price"]
)


# ------------------------------------------------------------
# 6. FUNCTION TO RUN ONE INSTITUTIONAL SCENARIO
# ------------------------------------------------------------

def run_scenario(
    detection_probability
):
    """
    Run the full 100-tender procurement market
    for one value of detection probability p.

    Everything else remains fixed.
    """

    scenario_market = market.copy()


    # --------------------------------------------------------
    # Firm corruption payoff
    # --------------------------------------------------------
    #
    # EU(Bribe) =
    # PrivateProfit - Bribe - p * FirmPenalty
    # --------------------------------------------------------

    scenario_market[
        "bribe_payoff"
    ] = (
        scenario_market[
            "private_profit"
        ]
        - BRIBE
        - detection_probability
        * FIRM_PENALTY
    )


    scenario_market[
        "willing_to_bribe"
    ] = (
        scenario_market[
            "bribe_payoff"
        ]
        > 0
    )


    # --------------------------------------------------------
    # Official decision
    # --------------------------------------------------------

    official_accept_payoff = (
        BRIBE
        - detection_probability
        * OFFICIAL_PENALTY
    )

    official_prefers_accept = (
        official_accept_payoff > 0
    )


    # --------------------------------------------------------
    # Process all tenders
    # --------------------------------------------------------

    tender_records = []


    for tender_id, tender_df in (
        scenario_market
        .groupby(
            "tender_id",
            sort=True
        )
    ):

        tender_df = tender_df.copy()


        # ----------------------------------------------------
        # Honest winner
        # ----------------------------------------------------

        honest_winner = (
            tender_df
            .sort_values(
                "public_value",
                ascending=False
            )
            .iloc[0]
        )

        honest_public_value = (
            honest_winner[
                "public_value"
            ]
        )


        # ----------------------------------------------------
        # Losing firms
        # ----------------------------------------------------

        losing_firms = (
            tender_df[
                tender_df["firm_id"]
                != honest_winner["firm_id"]
            ]
            .copy()
        )


        # ----------------------------------------------------
        # Firms willing to bribe
        # ----------------------------------------------------

        bribing_candidates = (
            losing_firms[
                losing_firms[
                    "willing_to_bribe"
                ]
            ]
            .copy()
        )


        # ----------------------------------------------------
        # Default outcome
        # ----------------------------------------------------

        final_winner = honest_winner

        corruption_attempted = False
        corruption_accepted = False
        award_distorted = False

        corrupt_candidate = None


        # ----------------------------------------------------
        # Strategic corruption
        # ----------------------------------------------------

        if len(
            bribing_candidates
        ) > 0:

            corruption_attempted = True

            corrupt_candidate = (
                bribing_candidates
                .sort_values(
                    "bribe_payoff",
                    ascending=False
                )
                .iloc[0]
            )


            # ------------------------------------------------
            # Official response
            # ------------------------------------------------

            if official_prefers_accept:

                corruption_accepted = True

                final_winner = (
                    corrupt_candidate
                )

                award_distorted = (
                    final_winner["firm_id"]
                    != honest_winner[
                        "firm_id"
                    ]
                )


        # ----------------------------------------------------
        # Final Public Value
        # ----------------------------------------------------

        final_public_value = (
            final_winner[
                "public_value"
            ]
        )


        # ----------------------------------------------------
        # Corruption loss
        # ----------------------------------------------------

        corruption_loss = (
            honest_public_value
            - final_public_value
        )


        if (
            corruption_loss < 0
            and np.isclose(
                corruption_loss,
                0
            )
        ):
            corruption_loss = 0.0


        assert (
            corruption_loss >= 0
        )


        # ----------------------------------------------------
        # Save tender result
        # ----------------------------------------------------

        tender_records.append(
            {
                "tender_id":
                    tender_id,

                "honest_public_value":
                    honest_public_value,

                "final_public_value":
                    final_public_value,

                "corruption_attempted":
                    corruption_attempted,

                "corruption_accepted":
                    corruption_accepted,

                "award_distorted":
                    award_distorted,

                "corruption_loss":
                    corruption_loss
            }
        )


    # --------------------------------------------------------
    # Tender-level DataFrame
    # --------------------------------------------------------

    tender_results = pd.DataFrame(
        tender_records
    )


    # --------------------------------------------------------
    # Aggregate scenario results
    # --------------------------------------------------------

    honest_public_value = (
        tender_results[
            "honest_public_value"
        ].sum()
    )

    final_public_value = (
        tender_results[
            "final_public_value"
        ].sum()
    )

    corruption_loss = (
        tender_results[
            "corruption_loss"
        ].sum()
    )

    corruption_attempts = int(
        tender_results[
            "corruption_attempted"
        ].sum()
    )

    accepted_bribes = int(
        tender_results[
            "corruption_accepted"
        ].sum()
    )

    distorted_awards = int(
        tender_results[
            "award_distorted"
        ].sum()
    )


    # --------------------------------------------------------
    # Loss percentage
    # --------------------------------------------------------

    loss_percentage = (
        corruption_loss
        / honest_public_value
        * 100
    )


    # --------------------------------------------------------
    # Number of firms willing to bribe
    # --------------------------------------------------------

    firms_willing_to_bribe = int(
        scenario_market[
            "willing_to_bribe"
        ].sum()
    )


    # --------------------------------------------------------
    # Accounting validation
    # --------------------------------------------------------

    assert np.isclose(
        honest_public_value
        - final_public_value,
        corruption_loss
    )


    return {
        "p":
            detection_probability,

        "official_accept_payoff":
            official_accept_payoff,

        "official_prefers_accept":
            official_prefers_accept,

        "firms_willing_to_bribe":
            firms_willing_to_bribe,

        "corruption_attempts":
            corruption_attempts,

        "accepted_bribes":
            accepted_bribes,

        "distorted_awards":
            distorted_awards,

        "honest_public_value":
            honest_public_value,

        "final_public_value":
            final_public_value,

        "corruption_loss":
            corruption_loss,

        "loss_percentage":
            loss_percentage
    }


# ============================================================
# 7. RUN ALL DETECTION SCENARIOS
# ============================================================

scenario_results = []


for p in DETECTION_VALUES:

    result = run_scenario(
        detection_probability=p
    )

    scenario_results.append(
        result
    )


results = pd.DataFrame(
    scenario_results
)


# ------------------------------------------------------------
# 8. CLEAN NUMERICAL REPRESENTATION OF p
# ------------------------------------------------------------

results["p"] = (
    results["p"]
    .round(2)
)


# ------------------------------------------------------------
# 9. VALIDATE HONEST COUNTERFACTUAL
# ------------------------------------------------------------

assert np.allclose(
    results[
        "honest_public_value"
    ],
    HONEST_PUBLIC_VALUE_EXPECTED,
    atol=0.02
)


# ------------------------------------------------------------
# 10. VALIDATE EXPERIMENT 5 AT p = 10%
# ------------------------------------------------------------

baseline_10 = (
    results[
        np.isclose(
            results["p"],
            0.10
        )
    ]
    .iloc[0]
)


assert (
    baseline_10[
        "corruption_attempts"
    ]
    == 100
)

assert (
    baseline_10[
        "accepted_bribes"
    ]
    == 100
)

assert (
    baseline_10[
        "distorted_awards"
    ]
    == 100
)

assert np.isclose(
    baseline_10[
        "final_public_value"
    ],
    5653.88,
    atol=0.02
)

assert np.isclose(
    baseline_10[
        "corruption_loss"
    ],
    2543.17,
    atol=0.02
)


# ------------------------------------------------------------
# 11. VALIDATE THEORETICAL OFFICIAL THRESHOLD
# ------------------------------------------------------------
#
# At p = 20%:
#
# EU(Accept) =
# 20 - 0.20*100
# = 0
#
# Since our decision rule is > 0,
# official must reject.
# ------------------------------------------------------------

threshold_20 = (
    results[
        np.isclose(
            results["p"],
            0.20
        )
    ]
    .iloc[0]
)


assert np.isclose(
    threshold_20[
        "official_accept_payoff"
    ],
    0
)

assert (
    bool(
        threshold_20[
            "official_prefers_accept"
        ]
    )
    is False
)

assert (
    threshold_20[
        "accepted_bribes"
    ]
    == 0
)

assert (
    threshold_20[
        "distorted_awards"
    ]
    == 0
)

assert np.isclose(
    threshold_20[
        "final_public_value"
    ],
    HONEST_PUBLIC_VALUE_EXPECTED,
    atol=0.02
)


# ============================================================
# 12. SAVE RESULTS
# ============================================================

results.to_csv(
    "results/detection_policy_sweep.csv",
    index=False
)


# ============================================================
# 13. FIGURE — CORRUPTION ACTIVITY VS DETECTION
# ============================================================

plt.figure(
    figsize=(10, 6)
)

plt.plot(
    results["p"],
    results["corruption_attempts"],
    marker="o",
    label="Corruption attempts"
)

plt.plot(
    results["p"],
    results["accepted_bribes"],
    marker="o",
    label="Accepted bribes"
)

plt.plot(
    results["p"],
    results["distorted_awards"],
    marker="o",
    label="Distorted awards"
)

plt.axvline(
    0.20,
    linestyle="--",
    label="Official threshold (20%)"
)

plt.xlabel(
    "Probability of detection"
)

plt.ylabel(
    "Number of tenders"
)

plt.title(
    "Kairos Corruption Lab — "
    "Corruption Activity vs Detection"
)

plt.legend()

plt.tight_layout()

plt.savefig(
    "figures/"
    "corruption_vs_detection.png",
    dpi=300
)

plt.close()


# ============================================================
# 14. FIGURE — PUBLIC VALUE VS DETECTION
# ============================================================

plt.figure(
    figsize=(10, 6)
)

plt.plot(
    results["p"],
    results["final_public_value"],
    marker="o",
    label="Observed Public Value"
)

plt.axhline(
    HONEST_PUBLIC_VALUE_EXPECTED,
    linestyle="--",
    label="Honest benchmark"
)

plt.axvline(
    0.20,
    linestyle="--",
    label="Official threshold (20%)"
)

plt.xlabel(
    "Probability of detection"
)

plt.ylabel(
    "Total Public Value"
)

plt.title(
    "Kairos Corruption Lab — "
    "Public Value vs Detection"
)

plt.legend()

plt.tight_layout()

plt.savefig(
    "figures/"
    "public_value_vs_detection.png",
    dpi=300
)

plt.close()


# ============================================================
# 15. FIGURE — CORRUPTION LOSS VS DETECTION
# ============================================================

plt.figure(
    figsize=(10, 6)
)

plt.plot(
    results["p"],
    results["corruption_loss"],
    marker="o"
)

plt.axvline(
    0.20,
    linestyle="--",
    label="Official threshold (20%)"
)

plt.axhline(
    0,
    linestyle="--"
)

plt.xlabel(
    "Probability of detection"
)

plt.ylabel(
    "Total Public Value Loss"
)

plt.title(
    "Kairos Corruption Lab — "
    "Corruption Loss vs Detection"
)

plt.legend()

plt.tight_layout()

plt.savefig(
    "figures/"
    "corruption_loss_vs_detection.png",
    dpi=300
)

plt.close()


# ============================================================
# 16. TERMINAL OUTPUT
# ============================================================

print(
    "\n"
    + "=" * 84
)

print(
    "KAIROS CORRUPTION LAB"
)

print(
    "EXPERIMENT 6 — "
    "INSTITUTIONAL RESPONSE TO DETECTION"
)

print(
    "=" * 84
)


print(
    "\nFIXED PARAMETERS"
)

print(
    f"Bribe: "
    f"{BRIBE:.2f}"
)

print(
    f"Firm penalty: "
    f"{FIRM_PENALTY:.2f}"
)

print(
    f"Official penalty: "
    f"{OFFICIAL_PENALTY:.2f}"
)

print(
    f"Private profit margin: "
    f"{PRIVATE_PROFIT_MARGIN:.0%}"
)

print(
    f"Honest Public Value benchmark: "
    f"{HONEST_PUBLIC_VALUE_EXPECTED:.2f}"
)


print(
    "\n"
    + "=" * 84
)

print(
    "DETECTION POLICY SWEEP"
)

print(
    "=" * 84
)


print(
    f"{'p':>6} "
    f"{'EU Official':>12} "
    f"{'Firms willing':>14} "
    f"{'Attempts':>10} "
    f"{'Accepted':>10} "
    f"{'Distorted':>10} "
    f"{'Public Value':>14} "
    f"{'Loss':>10} "
    f"{'Loss %':>9}"
)


print(
    "-" * 104
)


for _, row in results.iterrows():

    print(
        f"{row['p']:>6.0%} "
        f"{row['official_accept_payoff']:>12.2f} "
        f"{int(row['firms_willing_to_bribe']):>14} "
        f"{int(row['corruption_attempts']):>10} "
        f"{int(row['accepted_bribes']):>10} "
        f"{int(row['distorted_awards']):>10} "
        f"{row['final_public_value']:>14.2f} "
        f"{row['corruption_loss']:>10.2f} "
        f"{row['loss_percentage']:>8.2f}%"
    )


print(
    "=" * 104
)


# ------------------------------------------------------------
# 17. KEY RESULTS
# ------------------------------------------------------------

print(
    "\nKEY VALIDATIONS"
)


print(
    "\np = 10% reproduces Experiment 5:"
)

print(
    f"Attempts: "
    f"{int(baseline_10['corruption_attempts'])}"
)

print(
    f"Accepted: "
    f"{int(baseline_10['accepted_bribes'])}"
)

print(
    f"Distorted: "
    f"{int(baseline_10['distorted_awards'])}"
)

print(
    f"Public Value: "
    f"{baseline_10['final_public_value']:.2f}"
)

print(
    f"Corruption Loss: "
    f"{baseline_10['corruption_loss']:.2f}"
)


print(
    "\np = 20%:"
)

print(
    f"Official EU(Accept): "
    f"{threshold_20['official_accept_payoff']:.2f}"
)

print(
    f"Accepted bribes: "
    f"{int(threshold_20['accepted_bribes'])}"
)

print(
    f"Distorted awards: "
    f"{int(threshold_20['distorted_awards'])}"
)

print(
    f"Public Value: "
    f"{threshold_20['final_public_value']:.2f}"
)


print(
    "\nAll theoretical and accounting "
    "validations passed correctly."
)


# ------------------------------------------------------------
# 18. FILES GENERATED
# ------------------------------------------------------------

print(
    "\nFILES GENERATED"
)

print(
    "results/"
    "detection_policy_sweep.csv"
)

print(
    "figures/"
    "corruption_vs_detection.png"
)

print(
    "figures/"
    "public_value_vs_detection.png"
)

print(
    "figures/"
    "corruption_loss_vs_detection.png"
)


print(
    "\nExperiment 6 completed correctly."
)

print(
    "=" * 84
    + "\n"
)